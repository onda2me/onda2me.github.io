...
> **컴퓨터 비전(Computer Vision)**은 컴퓨터가 인간의 시각 시스템처럼 디지털 이미지나 비디오 등 시각적 데이터를 인식하고 처리하며, 그로부터 의미 있는 정보를 추출하고 이해하는 인공지능(AI)의 한 분야이다.
즉, 기계에 '보는 능력'을 부여하고, 본 것을 '이해'하게 만드는 기술이다.    
**Google Cloud Vision API**는 이미지 분석 및 이해를 위한 머신러닝 서비스로 개발자가 애플리케이션에 손쉽게 이미지 인식 기능을 통합할 수 있도록 서비스를 제공한다.
{: .notice--info}
